...
cat("\\begin{flushleft}{\\footnotesize Table 1: \\textit{Summary statistics for our response variable, the 1000-hour dead fuel moisture content (fm1000).}}\\end{flushleft}")
```
...
The Köppen-Geiger climate zones are based on seasonal precipitation and temperature (Kottek et. Al, 2006). We categorized each observation by climate zone since it’s more meaningful than raw latitude and longitude and included the interactions for pr\*cli, tmmn\*cli, and tmmx\*cli (Table 3).  
...
Astronomical seasons are based on the Earth’s orbit, which affects solar radiation received (U.S. Department of Commerce, n.d.). We categorized each observation by season and included the interaction srad*season (Table 4).  
...
In conclusion, since the dataset meets the ethical criteria (consent, privacy, and ownership), our dataset can not only be considered highly trustworthy, but our analysis of it is in line with the original sources of data.  
...
cat("\\begin{flushleft}{\\footnotesize Figure 1: \\textit{Response vs Fitted Values, Residuals vs Fitted Values, and Normal-QQ Plots for our preliminary model.}}\\end{flushleft}")
```
